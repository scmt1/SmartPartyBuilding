<template>
    <el-dialog :visible.sync="show" title="新增工会活动" top="20px" width="900px" :close-on-click-modal="false"
               :close-on-press-escape="false" @close="close">
        <el-form ref="form" :model="form" :rules="rules" :disabled="disabled" v-if="show">
            <div class="body">
                <el-form-item label="所属工会：" :label-width="formLabelWidth" prop="tradeName">
                    <el-input :disabled="true" v-model="form.tradeName" size="small" placeholder="" autocomplete="off"
                              maxlength="80" class="input-row"></el-input>
                </el-form-item>
                <el-form-item label="活动名称：" :label-width="formLabelWidth" prop="activityName" class="input-row">
                    <el-input v-model="form.activityName" size="small" placeholder="" autocomplete="off" maxlength="80"
                              class="input-row"></el-input>
                </el-form-item>
                <el-form-item label="活动描述：" :label-width="formLabelWidth" prop="description" class="input-row">
                    <el-input rows="7" v-model="form.description" size="small" type="textarea" autocomplete="off"
                              maxlength="80" class="input-row"></el-input>
                </el-form-item>
                <el-form-item label="活动图片：" :label-width="formLabelWidth" prop="floatingOutType" class="input-row">
                    <div style="display: flex">
                        <!-- <div style="text-align: left; " v-if="form.imgUrl">
                             &lt;!&ndash;<img :src="defaultImg" v-if="form.imgUrl==null || form.imgUrl==undefined||form.imgUrl.length == 0" style="display: inline-block; cursor: pointer;width: 160px;">&ndash;&gt;
                             <img  :src=" base + form.imgUrl" style="display: inline-block; cursor: pointer;width: 160px;">
                         </div>-->
<!--                        <Upload
                            accept=".jpg,.png,.jpeg"
                            ref="upload"
                            ...
                            <div style="width: 100px;height:100px;">
                                <img v-if="form.imgUrl" :src=" base + form.imgUrl"
                                     style="display: inline-block; cursor: pointer;width:100px;height:100px; object-fit: contain;">
                                <img v-else :src="defaultImg"
                                     style="display: inline-block; cursor: pointer;width:100px;height:100px;">
                            </div>
                        </Upload>-->
                        <el-upload
                            accept=".jpg,.png,.jpeg"
                    ...
                    <div v-if="fileList!=null&&fileList.length>0" style="text-align:left">
                        <div style="margin-top: 5px; display: flex;" v-for="(item, index) in fileList" :key="index">
                            <div style="flex: 1;">
                                ...
                                <i style="margin-left: 10px; cursor: pointer;" @click="downFile(item)"
                                   class="el-icon-download"></i>
                            </div>
                            <div style="margin-left: 15px; cursor: pointer;"><i @click="delFile(item)" class="el-icon-error"></i></div>
                        </div>
                    </div>
                </el-form-item>
            </div>
...
import {addTradeActive, getTradeActiveById} from '@/api/tzTradeActive'
import Vue from 'vue'
...
export default {
    ...
    methods: {
        ...
        getTradeActive(id) {
            getTradeActiveById(id).then(res => {
                if (res.data.code == '00000') {
                    this.form = res.data.data
                    this.uploadImageList = []
                    this.fileList = []

                    let imageFileList = JSON.parse(this.form.imgUrl)
            ...
            reader.onload = () => {
                ...
                uploadTinymceEditor({data: val}).then(res => {
                    ...
                    if (data.code = '00000') {
                        ...
                        attachFile.type = 3 // 文件 1:图片 2:视频 3:文件
                        attachFile.tableType = 'tz_trade_active'
                        this.uploadFileList.push(attachFile)
                    } else {
        ...
        submitForm() {
            ...
            this.$refs['form'].validate((valid) => {
                ...
                if (this.imageFileList.length == 0) {
                    this.$message.info("活动图片不能为空")
                    return false
                }
                let active = this.form
                active.fileUrl = this.fileList
                active.imgUrl = this.imageFileList
                addTradeActive({active: active}).then(res => {
                    if (res.data) {
                        this.$message({
                            message: '保存成功',
                            type: 'success',
    ...
    watch: {
        ...
        show(val) {
            if (val) {
                this.form.tradeName = this.tradeName
                this.form.tradeId = this.tradeId
                if (this.modeTitle == '编辑') {
                    this.getTradeActive(this.activeId)
                }
            } else {
                this.closeModal()
            }
