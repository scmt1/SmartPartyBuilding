...
public class UploadFileUtils {
    ...
    public static AttachFile uploadFile2(MultipartFile multipartFile, String fileName) throws IOException {
        String path = "";
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
        String DataStr = format.format(new Date());
        path = basePath + "" + DataStr + "/";
        File upload = FileUtil.upload2(multipartFile, path, fileName);
        String name = upload.getName();

        String filePath = "/tempfile/" + DataStr + "/" + name;

        AttachFile attachFile = new AttachFile();
        attachFile.setFilePath(filePath);
        attachFile.setFileSize(multipartFile.getBytes().length);
        attachFile.setFileType(name.split("\\.")[1]);
