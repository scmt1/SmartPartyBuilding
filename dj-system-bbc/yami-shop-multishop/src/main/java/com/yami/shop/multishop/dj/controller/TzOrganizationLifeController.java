...
import com.yami.shop.bean.dto.TaizhangDto;
...
public class TzOrganizationLifeController {
    ...
    public ServerResponseEntity<Object> addTzOrganizationLife(@RequestBody Map<String, Object> map) {
        try {
            String json1 = JSON.toJSONString(map.get("tzOrganizationLife"));
            String json2 = JSON.toJSONString(map.get("attachFile"));
            TzOrganizationLife tzOrganizationLife = JSON.parseObject(json1, TzOrganizationLife.class);
            AttachFile attachFile = JSON.parseObject(json2, AttachFile.class);
            ...
            //    return ServerResponseEntity.showFailMsg("保存失败");
            //}
            attachFile.setForeignKey(tzOrganizationLife.getId().toString());
            attachFile.setUploadTime(new Date());
            QueryWrapper<AttachFile> attachFileQueryWrapper = new QueryWrapper<>();
            attachFileQueryWrapper.eq("tz_attach_file.foreign_key", tzOrganizationLife.getId()).eq("tz_attach_file.table_type", "tz_organization_life");
            attachFileService.remove(attachFileQueryWrapper);
    ...
    public ServerResponseEntity<Object> deleteTzOrganizationLife(String[] ids) {
        ...
        try {
            ...
            if (res) {
                UpdateWrapper<TzPersonMeeting> meetingUpdateWrapper = new UpdateWrapper<>();
                meetingUpdateWrapper.set("tz_party_meeting.del_flag", 1).in("tz_party_meeting.meeting_id", ids);
                boolean res1 = personMeetingService.update(meetingUpdateWrapper);
                ...
                queryWrapper.and(i -> i.eq("tz_attach_file.type", "1").or(i2 -> i2.eq("tz_attach_file.type", "3")));
    ...
    public ServerResponseEntity<Object> getTzOrganizationLife(@RequestParam(name = "id") String id, String deptId) {
        ...
        try {
            ...
            if (res != null) {
                ...
                AttachFile one = attachFileService.getOne(queryWrapper1);
                map.put("mainImg",one);
                //会议应到人数查询
                //根据部门id查出所有党员
     ...
     * @return 返回获取结果
     */
    @ApiOperation("学习资料台账查询")
    @GetMapping("querytaizhang")
    public ServerResponseEntity<Object> querytaizhang(TzOrganizationLife tzOrganizationLife, SearchVo searchVo, PageVo pageVo) {
        ...
        tzOrganizationLife.setUsername(username);
        try {
            Map<String, Object> result = tzOrganizationLifeService.querytaizhang(tzOrganizationLife, searchVo, pageVo);
            return ServerResponseEntity.success(result);
        } catch (Exception e) {
            e.printStackTrace();
            return ServerResponseEntity.showFailMsg("查询异常:" + e.getMessage());
