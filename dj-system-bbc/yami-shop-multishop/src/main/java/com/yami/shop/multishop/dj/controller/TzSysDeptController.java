  ...
  /*  @Autowired
    ...
    public ServerResponseEntity<Object> getTzSysDeptNameList(String id) {
        try {
            ...
            //ShopDetail shopDetailByShopId = shopDetailService.getById(SecurityUtils.getShopUser().getShopId());
            String deptId = shopEmployee.getDeptId();
           /* String tenantId = shopEmployee.getTenantId();
            if(StringUtils.isBlank(tenantId)){
                return ServerResponseEntity.showFailMsg("租户id为空，请联系管理员");
            }*/
            Integer type = shopEmployee.getType();
            List<TzSysDept> list = new ArrayList<>();
           /* if (type == 0) {
                list = tzSysDeptService.getNameList("1",null);
            } else {
                if (StringUtils.isNotBlank(deptId)) {
                    list = tzSysDeptService.getNameList(deptId,null);
                }else {
                    return ServerResponseEntity.showFailMsg("暂无权限");
                }
            }*/
    ...
    /**
     * 功能描述：毕昇系统调用的接口,查询整个组织id
     *
     * @param
     * @return 返回获取结果
     */
    @ApiOperation("毕昇系统调用的接口，查询整个组织id")
    @GetMapping("getAllDeptName")
    public ServerResponseEntity<Object> getAllDeptName() {
        try {
            List<TzSysDept> nameList = tzSysDeptService.getAllName();
            if (nameList != null) {
                return ServerResponseEntity.success(nameList);
            } else {
                return ServerResponseEntity.showFailMsg("查询失败");
            }
        } catch (Exception e) {
