...
watch(
  ...
  (newData: any, oldData) => {
    try {
      props.chartConfig.option.series[3].data[0].coords = newData.coords1;
      props.chartConfig.option.series[4].data[0].coords = newData.coords2;
      props.chartConfig.option.series[5].data[0].coords = newData.coords3;
    } catch (error) {
      console.log(error)
    }
  },
