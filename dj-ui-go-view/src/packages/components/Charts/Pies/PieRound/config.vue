...
<template>
    ...
    <CollapseItem v-for="(item, index) in config.series" :key="index" :name="`圆环`" :expanded="true">
      ...
      <SettingItemBox name="渐变颜色">
        <SettingItem name="0%处的颜色">
          <n-color-picker 
            size="small" 
            :modes="['hex']" 
            v-model:value="item.data[0].itemStyle.normal.color.colorStops[0].color"
            ></n-color-picker>
        </SettingItem>
        <SettingItem name="100%处的颜色">
          <n-color-picker
            size="small"
            :modes="['hex']"
            v-model:value="item.data[0].itemStyle.normal.color.colorStops[1].color"
          ></n-color-picker>
        </SettingItem>
      </SettingItemBox>
      <!-- 中心标题 -->
