// @ts-ignore
import axiosInstance from './djAxios'
import {
  RequestHttpEnum,
  ContentTypeEnum,
  RequestBodyEnum,
...
export const customizeHttp = (targetParams: RequestConfigType, globalParams: RequestGlobalConfigType) => {
  ...
  // data 参数
  let data: RequestParamsObjType | FormData | any
  // params 参数
  ...
  switch (requestParamsBodyType) {
    ...
    case RequestBodyEnum.JSON:
      ...
      // json 赋值给 data
      const tenantId:any = window.localStorage.getItem('tenantId')
      const tmp =  tenantId.slice(1,tenantId.length-1)
      data.bizParams.bizTenantId = tmp
      break
