...
<script>
import {getTzSysOrgList, getTzSysOrg, getOrgListByName} from "@/api/cqSysOrg";
...
export default {
    ...
    methods: {
        ...
        getDeptList() {
            ...
            getOrgListByName(data).then(res =>{
                const data = res.data
                if (data.code == '00000') {
        ...
        getTzSysDeptForExpanded(deptId) {
            ...
            getTzSysOrg(deptId).then(res =>{
                const reslut = res.data
                if (reslut.code = '00000') {
                    let parentIds = reslut.data.orgLevel.split('-')
                ...
                if (reslut.code = '00000') {
                    ...
                    if (this.deptId !=null &&  this.deptId &&  this.deptId.length > 0) {
                        if (this.deptId != this.deptInfo.id) {
                            getTzSysOrg(this.deptId +'').then(res2 => {
        ...
        treeLoadNode(node, resolve){
            if(node.data.id){
                getTzSysOrgList(node.data.id+'').then(res => {
