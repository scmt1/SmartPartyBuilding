<template>
    <div style="height: 100%; background-color: white; padding: 15px; float: left; width: 100%;">
        <div style="text-align: left">
            ...
            <div class="search-div">考试状态：
                <el-select v-model="form.testType" size="small" placeholder="请选择试题类型">
                    <el-option v-for="(item,index) in testType" :key="index" :label="item.title" :value="item.value"></el-option>
                </el-select>
            </div>
            <div class="search-div">创建时间：
                ...
                <el-button size="small" @click="search" style="background: #b20000;color:#ffffff;margin-left:20px">搜索</el-button>
                <el-button size="small" @click="reset" style="background: #b20000;color:#ffffff;margin-left:20px">重置</el-button>
            </div>

        </div>
        <div class="con-header">
            <el-button plain size="small" icon="el-icon-plus" style="background: #b20000;color:#ffffff;text-align: left" @click="add">新增</el-button>
            <!--<el-button size="small" style="background: #b20000;color:#ffffff;" plain icon="el-icon-plus" @click="addQuestion">添加试题</el-button>-->
        </div>
        ...
        <div v-if="tableDataNow!=null&&tableDataNow.length" style="margin-top: 10px; float: left; width: 100%; ">
            <div style="width: 25%; padding: 10px; float: left; min-width: 300px;" v-for="(item, index) in tableDataNow" :key="index" >
                <div class="content-box">
                    ...
                    <div style="margin-top: 3px; margin-left:3px;word-break: break-all;">
                        <el-row :gutter="10" style="font-weight: bold;margin-top:10px">
                            <div style="text-align: left;">关联题库 &nbsp党建专题题库</div>
                        </el-row>
                        <el-row :gutter="10" style="font-weight: bold;margin-top:10px">
                            <div style="text-align: left">考试时长 &nbsp 240 &nbsp(分钟)</div>
                        </el-row>
                        <el-row :gutter="10" style="font-weight: bold;margin-top:10px">
                            <div style="text-align: left">参与人数 &nbsp{{ item.detailNum?item.detailNum:0 }} &nbsp(人)</div>
                        </el-row>
                    </div>
                        ...
                        <template v-if="item.voteStatus == 1">
                            <div class="row">
                                <el-row style="width: 100%;">
                                    <el-col :span="12" >
                                        <div class="col1" @click="edit(item.id)">编辑考试</div>
                                    </el-col>
                                    <el-col :span="12">
                                        <div class="col2" @click="goDetail(item.id)">发布考试</div>
                                    </el-col>
                                </el-row>
                            </div>
                            <div class="row">
                            <el-row style="width: 100%;">
                                <el-col :span="12">
                                    <div class="col1" @click="deleteById(item.id)">
                                        删除考试
                                    </div>
                                </el-col>
                                <el-col :span="12">
                    ...
                    <!--    <template v-else-if="item.voteStatus == 2">
                      ...
                      <!--  <div class="row">
                            &lt;!&ndash;<div class="row-left" @click="voteCalculate(item)">查看排名</div>&ndash;&gt;
...
<script>
import add from './addExam'
import {queryExamByPage} from "@/api/tzExam";
    ...
    methods: {
        ...
        getDataById() {
         ...
         /*   let data = {
            ...
            this.whereMap.pageNumber = this.pageNumber
            this.whereMap.pageSize = this.pageSize
            let data = this.whereMap
            queryExamByPage({data:data}).then(res => {
