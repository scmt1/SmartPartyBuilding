...
<script>
	...
	import {getAreaTreeAll} from "@/api/jcxfSysArea.js";
	export default {
		props: {
			show: {
				type: Boolean,
		...
		methods: {
			...
			formatData() {
				getAreaTreeAll().then(res =>{
					...
					this.town = area.map(t => t.children.map(v => v.children.map(i => i.children.map(n => {
						return {
							name: n.name,
							areaId: n.id
						}
					...
					switch (this.type) {
						...
						case 3:
							this.columns = [
								...
								this.town[0][0][0].map(res => res.name)
							]
							this.type4()
							break;
						default:
