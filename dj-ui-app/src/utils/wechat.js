import {createJsapiSignature} from '@/api/wxService'
const config = require('./config.js')
// #ifdef H5
var jwx = require('./jwx/index.js')
// #endif
...
export default {
  ...
  getWxConfig(cb) {
    ...
    createJsapiSignature({url:url}).then(res =>{
      if(res.code=='00000'){
        let data = res.data
        jwx.config({
          ...
          jsApiList: ['updateAppMessageShareData','updateTimelineShareData', 'onMenuShareAppMessage', 'scanQRCode', 'getLocation'] // 必填，需要使用的JS接口列表，所有JS接口列表见附录2
        })
        if (cb) {
          cb()
        }
  ...
  callWexinPay(paydata, cb, errorCb) {
    ...
    const packages = paydata.package
